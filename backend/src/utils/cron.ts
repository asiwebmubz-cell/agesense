...
export const initCronJobs = () => {
  ...
  cron.schedule('0 0 * * *', async () => {
    ...
    try {
      ...
      const revokedResult = await db.query('DELETE FROM revoked_tokens WHERE expires_at < NOW()');
      logger.info(`Deleted ${revokedResult.rowCount} expired revoked tokens.`);

      // 2. Clean up expired refresh tokens
      const refreshResult = await db.query('DELETE FROM refresh_tokens WHERE expires_at < NOW()');
      logger.info(`Deleted ${refreshResult.rowCount} expired refresh tokens.`);

      // 3. Clean up security logs older than 180 days
      const logsResult = await db.query("DELETE FROM security_logs WHERE created_at < NOW() - INTERVAL '180 days'");
