      ...
      <header className="max-w-[1200px] mx-auto px-4 md:px-8 pt-4 pb-10 space-y-6">
        <div className="flex flex-wrap items-center gap-3 text-sm font-bold tracking-widest uppercase text-primary">
          ...
          <span className="text-on-surface-variant">
            Published on {program.created_at ? new Date(program.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'Draft / Unsaved'}
          </span>
        </div>
      ...
      <main className="max-w-[1200px] mx-auto px-4 md:px-8 space-y-16">
        ...
        {allImages.length > 0 ? (
          <section className="w-full">
            <ShowcaseGallery images={allImages} title={program.title || "Untitled"} />
          </section>
        ) : (
          <div className="border border-dashed border-outline-variant rounded-2xl p-12 text-center text-on-surface-variant bg-surface-container-low">
            <span className="material-symbols-outlined text-5xl text-outline mb-2">collections</span>
            <p className="text-sm font-medium">No images uploaded or selected yet.</p>
          </div>
        )}
